...
// '아이디와 비밀번호가 모두 입력되었는가?'를 판단하는 계산 속성 => 사용자ID와 PW 둘다 입력돼야 로그인버튼 활성화.
//  둘 다 입력되지 않으면 true → 로그인 버튼 비활성화
const disableSubmit = computed(() => !(member.username && member.password));

//username, password입력하고 login을 클릭하면 호출되는 함수
const login = async () => {
  console.log(member);
  try {
    await auth.login(member); //스토어의 login액션 호출
    router.push('/'); //로그인 성공시 홈으로 이동
  } catch (e) {
    //로그인 에러
    console.log('에러=======', e);
    error.value = e.response.data; //서버 에러메시지 출력.
  }
};
</script>
